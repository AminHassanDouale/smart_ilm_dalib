...
new class extends Component {
    ...
    public function students(): LengthAwarePaginator
    {
        return Student::query()
            ...
            ->when($this->studentId, fn(Builder $q) => $q->where('students.studentId', 'like', "%$this->studentId%"))
    ...
    public function headers(): array
    {
        return [
            ...
            ['key' => 'studentId', 'label' => 'Numero d\'Etudiant'],
            ['key' => 'name', 'label' => 'Nom'],
            ['key' => 'billMethod.name', 'label' => 'Méthode de Facturation'],
            ['key' => 'filiere.name', 'label' => 'Filière'],
            ['key' => 'niveau.name', 'label' => 'Niveau'],
            ['key' => 'section.name', 'label' => 'Section'],
            ['key' => 'status', 'label' => 'Statut', 'sortBy' => 'status_name', 'class' => 'hidden lg:table-cell'],
...
<div>
    <x-header title="Etudiant(e)s" separator progress-indicator>
        {{-- SEARCH --}}
        <x-slot:middle class="!justify-end">
            <x-input placeholder="Nom de l'etudiant..." wire:model.live.debounce="name" icon="o-magnifying-glass" clearable />
        </x-slot:middle>
        ...
        <x-slot:actions>
            ...
            <x-button label="exception" icon="c-arrow-right" link="/students/exception" class="btn-primary" responsive />
        </x-slot:actions>
    </x-header>
    <x-card>
        @if($students->count() > 0)
        <x-table :headers="$headers" :rows="$students" link="/students/{id}/detail" :sort-by="$sortBy" with-pagination>
        ...
        <div class="flex items-center justify-center gap-10 mx-auto">
            ...
            <div class="text-lg font-medium">
                Desole {{ Auth::user()->name }}, Il y a Aucun(e) Etudiant(e) .
            </div>
        </div>
        @endif
    </x-card>
    ...
    <x-drawer wire:model="showFilters" title="Filters" class="lg:w-1/3" right separator with-close-button>
        <div class="grid gap-5" @keydown.enter="$wire.showFilters = false">
            ...
            <x-select label="Niveau" :options="$niveaux" wire:model.live="niveau_id" icon="c-adjustments-vertical" placeholder="All"
                ...
                placeholder-value="0" inline />
            <x-select label="Méthode de Facturation" :options="$billMethods" wire:model.live="billmethod_id" icon="c-adjustments-horizontal" placeholder="All"
                placeholder-value="0" inline />
        </div>

        {{-- ACTIONS --}}
        <x-slot:actions>
            <x-button label="Réinitialiser" icon="o-x-mark" wire:click="clear" spinner />
            <x-button label="Terminé" icon="o-check" class="btn-primary" @click="$wire.showFilters = false" />
        </x-slot:actions>
    </x-drawer>
</div>
